...
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 Iniciando aprendizado intensivo simplificado...')
    ...
    // 2. Algoritmo de aprendizado intensivo
    ...
    console.log(`✅ Aprendizado intensivo concluído - Precisão: ${accuracy.toFixed(2)}%`)
    
    // 4. Gerar pares simulados para treinamento adicional
    console.log('🔄 Gerando 1000 pares simulados...')
    const simulatedPairs = generateSimulatedPairs(1000, allCandles)
    ...
    return NextResponse.json({
      success: true,
      message: 'Aprendizado intensivo simplificado concluído!',
      stats: {
    ...
    console.error('Erro no aprendizado intensivo simplificado:', error)
    return NextResponse.json({
      success: false,
      error: 'Erro no aprendizado intensivo simplificado',

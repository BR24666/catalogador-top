  ...
  try {
    if (isRunning) {
      return NextResponse.json({
        success: true,
        message: 'Aprendizado contínuo ROBUSTO já está rodando!',
        isRunning: true,
        learningCount
      })
    }

    console.log('🚀 Iniciando aprendizado contínuo ROBUSTO...')
    isRunning = true
    learningCount = 0

    // Função de aprendizado contínuo robusta
    const continuousLearning = async () => {
      try {
        learningCount++
        console.log(`🧠 Executando ciclo de aprendizado #${learningCount}...`)
        ...
        for (const pair of FOCUSED_PAIRS) {
          try {
            const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${pair}&interval=1m&limit=300`)
        ...
        console.log(`📊 Coletados ${allCandles.length} velas de ${FOCUSED_PAIRS.length} pares`)
        ...
        console.log(`🧠 Ciclo #${learningCount} concluído - Precisão: ${finalAccuracy.toFixed(2)}%`)
    ...
    // Configurar intervalo de 20 segundos (mais frequente)
    learningInterval = setInterval(continuousLearning, 20000)
    
    return NextResponse.json({
      success: true,
      message: 'Aprendizado contínuo ROBUSTO iniciado!',
      isRunning: true,
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: 'Aprendizado contínuo ROBUSTO parado!',
      isRunning: false
    })
  } catch (error) {
    return NextResponse.json({
